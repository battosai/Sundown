...
public class PathFinding
{
    ...
    private Dictionary<int, int[]> splitDirections = new Dictionary<int, int[]>() 
    {
    ...
    public List<Vector2> JumpPoint(Vector2 destination, int nodeID)
    {
    ...
    public List<Vector2> AStar(Vector2 destination, int nodeID)
	{
		List<Vector2> path = new List<Vector2>();
		...
		while(stack.Count > 0)
		{
			...
			{
				List<Node> neighbors = GetNeighbors(currNode, nodeMap);
    ...
    public bool CheckNeighbor(Node origin, Node[,] nodeMap, int direction)
    {
        int row = origin.row;
        int col = origin.col;
	...
	public List<Node> GetNeighbors(Node origin, Node[,] nodeMap)
	{
		int row = origin.row;
		int col = origin.col;
    ...
    private Node[] bubbleSort(Node[] unsorted)
    {
